import { Buffer } from 'buffer';
import { beginCell, Cell, storeStateInit } from '@ton/core';

import { parseAddress } from '../core/address';
import { HighloadWalletV3 } from '../wallets/highload-v3/HighloadWalletV3';
import { WalletExecutionError } from './errors';
import type { HighloadWalletDescriptor } from './types';

const HIGHLOAD_WALLET_V3_CODE_HEX =
    '';

function highloadWalletCode(): Cell {
    return Cell.fromBoc(Buffer.from(HIGHLOAD_WALLET_V3_CODE_HEX, 'hex'))[0]!;
}

export function createHighloadWalletContract(
    wallet: HighloadWalletDescriptor,
    publicKey: Buffer,
): HighloadWalletV3 {
    return HighloadWalletV3.createFromConfig(
        {
            publicKey,
            subwalletId: wallet.subwalletId,
            timeout: wallet.timeoutSeconds,
        },
        highloadWalletCode(),
        0,
    );
}

export function assertHighloadWalletAuthority(
    wallet: HighloadWalletDescriptor,
    publicKey: Buffer,
): void {
    const contract = createHighloadWalletContract(wallet, publicKey);
    if (!parseAddress(wallet.address).equals(contract.address)) {
        throw new WalletExecutionError(
            'INVALID_WALLET_REQUEST',
            'The wallet descriptor does not match the decrypted signing authority.',
        );
    }
}

export function highloadTonConnectAddressPayload(
    wallet: HighloadWalletDescriptor,
    publicKey: Buffer,
): { readonly address: string; readonly walletStateInit: string } {
    const contract = createHighloadWalletContract(wallet, publicKey);
    if (!contract.init) {
        throw new WalletExecutionError('INVALID_WALLET_REQUEST', 'The highload wallet state init is unavailable.');
    }
    const walletStateInit = beginCell()
        .store(storeStateInit(contract.init))
        .endCell()
        .toBoc()
        .toString('base64');
    return Object.freeze({
        address: contract.address.toRawString(),
        walletStateInit,
    });
}
